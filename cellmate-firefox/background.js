...
chrome.webRequest.onBeforeSendHeaders.addListener(
  ...
  { urls: ["<all_urls>"] },
  // Firefox-specific: drop "extraHeaders". Chrome needs it to expose the
  // Cookie header here; Firefox includes Cookie unconditionally and rejects
  // the unknown enum value, which crashes listener registration.
  (() => {
    const spec = ["blocking", "requestHeaders"];
    try {
      const opts = chrome.webRequest?.OnBeforeSendHeadersOptions;
      if (opts && "EXTRA_HEADERS" in opts) spec.push("extraHeaders");
    } catch {}
    return spec;
  })()
);
